...
- Q: サブタスクは `Task` に `parent_task_id` を持たせる形で表現しますか、それとも別エンティティにしますか？ → A: `A`（`Task.parent_task_id` を用いる）
 ...
 - Q: 削除はソフトデリートして何日後にハードデリートしますか？ → A: `A (7 days)`（ソフトデリート → 7日後に完全削除）
...
- Deletion semantics
  - タスク削除時: ソフトデリート（`deleted_at` をセット）を行い、デフォルトで7日後に完全削除（ハードデリート）する。誤削除からの復元を容易にするためソフトデリートを採用する。管理者が保持期間を調整できる設定は将来的に検討する。
  - サブタスクの扱い: 削除時は子（parent_task_id に紐づくタスク）を同時にソフトデリートする（カスケード）。
  - タグは削除しない（タグは多くのタスクで共有されるため）。
- Concurrency
